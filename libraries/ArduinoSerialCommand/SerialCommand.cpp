...
void SerialCommand::readSerial()
{
	...
	#else
	while ((usingSoftwareSerial==0 && _HardSerial->available() > 0) || (usingSoftwareSerial==1 && SoftSerial->available() > 0) )
	#endif
	{
		int i;
        boolean matched;
		if (usingSoftwareSerial==0) {
			...
			inChar=_HardSerial->read();   // Read single available character, there may be more waiting
		} else {
			#ifndef SERIALCOMMAND_HARDWAREONLY
			// SoftwareSerial port
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
	if (numCommand < MAXSERIALCOMMANDS) {
		#ifdef SERIALCOMMANDDEBUG
		_HardSerial->print(numCommand);
		_HardSerial->print("-");
		_HardSerial->print("Adding command for ");
		...
		// the command
		#ifdef SERIALCOMMANDDEBUG
		_HardSerial->println("Too many handlers - recompile changing MAXSERIALCOMMANDS");
		#endif
	}
}
